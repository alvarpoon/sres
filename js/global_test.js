...
function getLevel() {
    ...
    levelArray.shift();
	//uncomment the following line if the root folder of the website is 1 level under the domain name. If it's 2 level under, add 2 lines
	// levelArray.shift();
...
function genTopNav(){
	...
	str +='<a href="http://www.xe.com/" target="_blank">Currency convertor</a>';
	str +='</div>';
	str +='</div>';
...
function initHomeSlideshow()
{
	...
	function goForward(){
		...
		if(isIE8){
			setTimeout('$("#homeMainContent .homeSlideshowItem").eq(0).css({"opacity":1, "filter": "alpha(opacity=100)", "-ms-filter": "progid:DXImageTransform.Microsoft.Alpha(Opacity=100)", "filter": "progid:DXImageTransform.Microsoft.Alpha(Opacity=100)"});', bannerContentDelay);
			setTimeout('$("#homeMainContent .homeSlideshowItem .homeContent").eq(0).show()', homeTitleDelay);
			setTimeout('$("#homeMainContent .homeSlideshowItem .homeText").eq(0).show()', homeTextDelay);
		}else{
...
{
//console.log("open");
browser_ref = window.open(url,'_blank','location=no,enableViewportScale=yes');
