...
function initTopNav(){
			...
			if($('#subNavContainer .active').length == 0){
				if($(this).hasClass('btnTools')){
					...
					if ($("#breadcumb").length > 0){
						$('#breadcumb').animate({'top':subNavHeight+80}, 150);
					}
					$(this).addClass('active');
					toolsShowFlag = false;
				}
				...
				if($(this).hasClass('btnTools')){
					...
					if ($('#breadcumb').length > 0){$('#breadcumb').animate({'top':subNavHeight+80},150);}
	...
	$('#subNavContainer .subNavTabContainer .subSectionItem').each(function(){
		$(this).click(function(){
			...
			if ($('#breadcumb').length > 0){$('#breadcumb').animate({'top':subNavHeight+80},350);}
	  });
	})
		...
		for(var j=0; j<levelArray.length; j++)
		{
			if(levelArray[j] != 'index')
			{
				...
				if(j == levelArray.length-1 || levelArray[j+1] == 'index'){
					...
					$('<a>',{
						 text: levelfilter[0].pageName,
						 href: homeHref+levelfilter[0].pageLink
					}).appendTo($('#breadcumb'));
				}
			}
			if(j != levelArray.length-1 && levelArray[j+1] != 'index')
	...
	if ($('#breadcumb').length > 0){
		...
		for(var j=0; j<levelArray.length; j++)
		{
			if(j == levelArray.length-1)
			{
				...
				$('<a>',{
					 text: levelfilter[0].levelName[j],
					 href: homeHref+levelfilter[0].levelLinks[j]
				}).appendTo($('#breadcumb'));
			}
			if(j != levelArray.length-1 && levelArray[j+1] != 'index')
			{
...
function initTrackRecord()
{
	...
	function initOverlay()
	{
		...
		function setContent(num)
		{
			...
			overlayLogo.src = trackRecordArray[num].thumb.replace('.png', '_overlay.png');
			companyNameDiv.innerHTML = trackRecordArray[num].companyName;
			
			//if(trackRecordArray[num].caseStudy != "")
			//{
			//	caseStudyBtn.href = trackRecordArray[num].caseStudy;
			//	caseStudyBtn.style.display = 'block';				
			//}
			
			//comment this due to filtering bug (20121018 11am)
			//if(trackRecordArray[num].branch.length > 1 && currentLocation == null)
			if(trackRecordArray[num].branch.length > 1)
			{
				branchBtnDiv.style.top = (400-trackRecordArray[num].branch.length*70)/2+'px';
				for(var i=0; i<trackRecordArray[num].branch.length; i++)
				{
					var branchContent = createDiv('', '', trackRecordArray[num].branch[i]);
					branchInfoDiv.appendChild(branchContent);
					
					var caseStudyBtn = createA(trackRecordArray[num].caseStudy[i], '', '', '', 'caseStudyBtn')
			...
			//}else if(currentLocation != null){
				//alert(include(trackRecordArray[num].location,currentLocation.toLowerCase()));
				//var a = trackRecordArray[num].location.indexOf(currentLocation.toLowerCase()); 				
			}else{
				var branchContent = createDiv('', '', trackRecordArray[num].branch[0]);
				branchContent.style.display = 'block';
				branchInfoDiv.appendChild(branchContent);
				
				var caseStudyBtn = createA(trackRecordArray[num].caseStudy[0], '', '', '', 'caseStudyBtn')
	...
	function showLogo(arr, imgClass, size)
	{
		...
		setTimeout(function() {
			...
			for(var f=0; f<arr.length; f++)
			{
				...
				{
					if(arr.length >= 5)
						var img = createImg(arr[f].thumb.replace('.png', '_lrg.png'), ' ', '', '', '', '', imgClass+' lrgLogoMargin', arr[f].id);
					else
						var img = createImg(arr[f].thumb.replace('.png', '_lrg.png'), ' ', '', '', '', '', imgClass, arr[f].id);
				}
				else
					var img = createImg(arr[f].thumb, ' ', '', '', '', '', imgClass,  arr[f].id);
				d.getElementById('trackLogoContainer').appendChild(img);
			}
			...
			div.style.paddingTop = Math.round((615-containerHeight)/2)+'px';
		...
		setTimeout(function() {
			...
			if(trackLogoArray.length < 5)
			{
				//864px
				//113px
				var marginValue = Math.floor(738-(113*trackLogoArray.length))/(trackLogoArray.length*2)+'px';
				for(var h=0; h<trackLogoArray.length; h++)
				{
					trackLogoArray[h].style.marginRight = marginValue;
					trackLogoArray[h].style.marginLeft = marginValue;
				}
			}
...
function initOurTeam()
{
	...
	function showLogo(arr, imgClass)
	{
		setTimeout(function() {
			for(var f=0; f<arr.length; f++)
			{
				var img = createImg('../images/ourTeam/'+arr[f].empName.toLowerCase().replace(/['"]/g,'')+'.png', ' ', '', '', '', '', imgClass,arr[f].id);
				d.getElementById('teamLogoContainer').appendChild(img);
			}
		}, 1000);
	...
	Array.prototype.locationFilter = function(a, searchStr) {
	  ...
	  for (i=0; i<this.length; i++) {
			...
			//if(this[i].location.toLowerCase() == searchStr.toLowerCase()) {
...
function initContactPopup()
{
	...
	defaultResearchReportArray = [{contactPersonImg:"images/img_sres_popup.jpg", contactPerson:"", cpTitle:"Contact the team at Strategic Real Estate Services", cpDepartment:"", cpTel:"+852 2842 4410", cpEmail:"scs@savills.com"}]
...
function initResearchReportPopup()
{
	...
	function setContent(num)
	{
		...
		$('#researchReportOverlay .btn_downloadPDF').attr("href" , researchReportArray[num].pdfLink);
		$('#researchReportOverlay .btn_downloadPDF').attr("target","_blank");
		$('#researchReportOverlay .btn_previewPDF').attr("href" , researchReportArray[num].previewLink);
...
function initLeftPanelDropDown(id)
{
	$('#'+id+' .regionDropdownHeader').toggle(
		function(){
			...
			{
				$('#'+id+' .regionDropdownContentMask').css('z-index',899);
			}else
				$('#'+id+' .regionDropdownContentMask').css('z-index',800);
	...
	$('.introButtonContainer .pm_intro_item').each(function(){
		$(this).click(function(){
			...
			/*if($('.introButtonContainer .pm_intro_item').index($(this)) == 0)
				$(this).animate({'margin':'0 35px 0 0'},300)
			else */if($('.introButtonContainer .pm_intro_item').index($(this)) == 3)
				$(this).animate({'margin':'0 0 0 35px'},300)
			else
				$(this).animate({'margin':'0 35px'},300)
	...
	function resetPMIntro()
	{
		$('.introButtonContainer .active').each(function(){
			...
			if($('.introButtonContainer .pm_intro_item').index($(this)) == 0)
				...
				$(this).animate({'margin':'0 15px'},300)
		})
...
function genTopNav(){
	...
	str +='<a href="#" class="btnTools"><span><img src="' + homeHref + 'images/text_tools.gif" alt="tools" /></span></a>';
	str +='<a href="#" class="btnShare"><span><img src="' + homeHref + 'images/text_share.gif" alt="share" /></span></a>';
	...
	str +='<a class="page-class-7 page-class-8 page-class-9 page-class-10 page-class-11 page-class-12 page-class-13 page-class-14 page-class-15 page-class-16 page-class-17" href="' + homeHref + 'who-we-are/savills-by-numbers/global-revenue-ranking.html">Savills by Numbers</a>';
	str +='</div>';
	str +='</div>';
	str +='</div>';
	str +='</div>';
	...
	str +='</div>';
	str +='<div class="subSectionItem">';
	str +='<a class="page-class-33" href="' + homeHref + 'where-we-play/america-footprint.html">Americas Footprint</a>';
	str +='<ul>';
	str +='<li><a class="page-class-34" href="' + homeHref + 'where-we-play/america-footprint-city-guide/atlanta.html">Atlanta</a></li>';
	str +='<li><a class="page-class-35" href="' + homeHref + 'where-we-play/america-footprint-city-guide/boston.html">Boston</a></li>';
	str +='<li><a class="page-class-36" href="' + homeHref + 'where-we-play/america-footprint-city-guide/chicago.html">Chicago</a></li>';
	str +='<li><a class="page-class-37" href="' + homeHref + 'where-we-play/america-footprint-city-guide/los-angeles.html">Los Angeles</a></li>';
	str +='<li><a class="page-class-38" href="' + homeHref + 'where-we-play/america-footprint-city-guide/new-york.html">New York</a></li>';
	str +='<li><a class="page-class-39" href="' + homeHref + 'where-we-play/america-footprint-city-guide/orange-county.html">Orange County</a></li>';
	str +='<li><a class="page-class-40" href="' + homeHref + 'where-we-play/america-footprint-city-guide/toronto.html">Toronto</a></li>';
	str +='<li><a class="page-class-41" href="' + homeHref + 'where-we-play/america-footprint-city-guide/washington-dc.html">Washington, DC</a></li>';
	str +='</ul>';
	str +='</div>';
	str +='<div class="subSectionItem lastItem">';
	...
	str +='<li><a class="page-class-48" href="' + homeHref + 'where-we-play/europe-footprint-city-guide/milan.html">Milan</a></li>';
	...
	str +='</div>';
	/*str +='<div class="subSectionItem">';
	str +='<a href="#">Earth<br />Consultancy</a>';
	str +='</div>';*/
	str +='<div class="subSectionItem lastItem">';
	...
	str +='<div class="subNavItem">';
	str +='<div class="subNavItemWrapper">';
	str +='<div class="sectionTitle">Our Research</div>';
	...
	str +='<span></span>';
	str +='</div>';
	/*str +='<div class="subSectionItem lastItem" id="industry-standards">';
	str +='<a href="javascript:;">Industry Standards</a>';
	str +='<span></span>';
	str +='</div>';*/
	str +='</div>';
	...
	str +='</div>';
	str +='<div class="subSectionItem">';
	str +='<a class="page-class-106" href="' + homeHref + 'our-research/city-briefings/americas/atlanta-briefing.html">Americas</a>';
	str +='<ul>';
	str +='<li><a class="page-class-106" href="' + homeHref + 'our-research/city-briefings/americas/atlanta-briefing.html">Atlanta</a></li>';
	str +='<li><a class="page-class-107" href="' + homeHref + 'our-research/city-briefings/americas/boston-briefing.html">Boston</a></li>';
	str +='<li><a class="page-class-108" href="' + homeHref + 'our-research/city-briefings/americas/chicago-briefing.html">Chicago</a></li>';
	str +='<li><a class="page-class-109" href="' + homeHref + 'our-research/city-briefings/americas/los-angeles-briefing.html">Los Angeles</a></li>';
	str +='<li><a class="page-class-110" href="' + homeHref + 'our-research/city-briefings/americas/new-york-briefing.html">New York</a></li>';
	str +='<li><a class="page-class-111" href="' + homeHref + 'our-research/city-briefings/americas/orange-county-briefing.html">Orange County</a></li>';
	str +='<li><a class="page-class-112" href="' + homeHref + 'our-research/city-briefings/americas/toronto-briefing.html">Toronto</a></li>';
	str +='<li><a class="page-class-113" href="' + homeHref + 'our-research/city-briefings/americas/Washington, DC-dc-briefing.html">Washington, DC</a></li>';
	str +='</ul>';
	str +='</div>';
	str +='<div class="subSectionItem lastItem">';
	...
	str +='<li><a class="page-class-119" href="' + homeHref + 'our-research/city-briefings/europe/milan-briefing.html">Milan</a></li>';
	...
	str +='</div>';
	str +='<div class="subSectionItem">';
	str +='<a class="page-class-132 page-class-133 page-class-134 page-class-135" href="' + homeHref + 'our-research/research-reports/americas/index.html">Americas</a>';
	str +='</div>';
	str +='<div class="subSectionItem lastItem">';
	str +='<a href="' + homeHref + 'our-research/research-reports/europe/index.html">Europe</a>';
	str +='</div>';
	str +='</div>';
	str +='<div class="subNavTabChildItem">';
	str +='<div class="subSectionItem">';
	str +='<a href="#">APAC standard</a>';
	str +='</div>';
	str +='<div class="subSectionItem">';
	str +='<a href="#">Americas standard</a>';
	str +='</div>';
	str +='<div class="subSectionItem lastItem">';
	str +='<a href="#">Europe standard</a>';
	str +='</div>';
	str +='</div>';
	str +='</div>';
	str +='</div>';
	...
	/*str +='<div class="subNavItem">';
	str +='<div class="subNavItemWrapper">';
	str +='<div class="sectionTitle">Our Clients</div>';
	str +='<div class="subNavTabContainer clearfix">';
	str +='<div class="subSectionItem currentTab">';
	str +='<a href="#">Track Record</a>';
	str +='<span></span>';
	str +='</div>';
	str +='<div class="subSectionItem">';
	str +='<a href="#">Case Studies</a>';
	str +='<span></span>';
	str +='</div>';
	str +='</div>';
	str +='<div class="subNavTabChildContainer clearfix">';
	str +='<div class="subNavTabChildItem" style="display:block;">';
	str +='<div class="subSectionItem">';
	str +='<a href="' + homeHref + 'our-clients/track-record/tr-apac/index.html">APAC</a>';
	str +='</div>';
	str +='<div class="subSectionItem">';
	str +='<a href="' + homeHref + 'our-clients/track-record/tr-americas/index.html">Americas</a>';
	str +='</div>';
	str +='<div class="subSectionItem lastItem">';
	str +='<a href="' + homeHref + 'our-clients/track-record/tr-europe/index.html">Europe</a>';
	str +='</div>';
	str +='</div>';
	str +='<div class="subNavTabChildItem">';
	str +='<div class="subSectionItem">';
	str +='<a href="' + homeHref + 'our-clients/case-studies/apac/beijing-legal.html">APAC</a>';
	str +='</div>';
	str +='<div class="subSectionItem">';
	str +='<a href="' + homeHref + 'our-clients/case-studies/americas/washington-technology.html">Americas</a>';
	str +='</div>';
	str +='<div class="subSectionItem lastItem">';
	str +='<a href="' + homeHref + 'our-clients/case-studies/europe/amsterdam-technology.html">Europe</a>';
	str +='</div>';
	str +='</div>';
	str +='</div>';
	str +='</div>';
	...
	str +='<li><a class="page-class-137" href="' + homeHref + 'our-clients/track-record/tr-americas/index.html">Americas</a></li>';
	...
	str +='<li><a class="page-class-158 page-class-159 page-class-160 page-class-161 page-class-162 page-class-163 page-class-164 page-class-165 page-class-166 page-class-167 page-class-168 page-class-169 page-class-170 page-class-171 page-class-172 page-class-173 page-class-174 page-class-175 page-class-176 page-class-177 page-class-178 page-class-179 page-class-180 page-class-181 page-class-182" href="' + homeHref + 'our-clients/case-studies/americas/atlanta-CredAbility.html">Americas</a></li>';
	...
	str +='<div class="subNavItemWrapper">';
	str +='<a href="#">Print this page</a>';
	str +='<a href="#">PDF download</a>';
	str +='<a href="' + homeHref + 'http://www.xe.com/" target="_blank">Currency convertor</a>';
	str +='</div>';
	str +='</div>';
	...
	str +='<a class="addthis_button_google_plusone_badge" g:plusone:size="small" g:plusone:name="" g:plusone:href="' + homeHref + 'https://plus.google.com/102383601500147943541/"></a> ';
	str +='</div>';
	str +='</div>';
	str +='<script type="text/javascript">var addthis_config = {"data_track_addressbar":false};</script>';
	str +='<script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-5153b9e640b238a8"></script>';
	str +='</div>';
	str +='<div class="subNavItem"></div>';
	str +='<div class="subNavItem searchSubNav">';
	str +='<input type="text" id="searchfield" style="background:url(../images/btn_search_lrg.gif) no-repeat right; width:815px; height:40px; border:none; border-bottom:1px solid #3C3C3C; padding:0; margin:35px 105px; font-size:25px; color:#797979; font-family:Arial, Helvetica, sans-serif;" value="Search"/>';
	str +='</div>';
	str +='</div>';
...
function initHomeSlideshow()
{
	...
	setTimeout('$("#homeMainContent .homeSlideshowItem").eq(0).animate({"opacity":1},500)', bannerContentDelay);
	setTimeout('$("#homeMainContent .homeSlideshowItem .homeContent").eq(0).fadeIn()', homeTitleDelay);
	setTimeout('$("#homeMainContent .homeSlideshowItem .homeText").eq(0).fadeIn()', homeTextDelay);
